...
- **Photo collage/montage (her Contact-page request) — partially resolved 2026-07-30:** Google Drive reconnected fine (confirmed working again via direct tool calls, even though the connector's own status check kept reporting "not connected" — that check isn't reliable, a real call is). Built a 6-photo gallery on `/contact` ("In the gym") using every photo in her "Pics for website" folder that's under the Drive tool's 10MB download cap — confirmed this cap is a genuine, permanent tool limitation (not a connection issue) by testing a borderline file and an 18.5MB file, both failed, then a small file immediately after succeeded. Only 1 of these 6 (narine-contact-1) actually shows her with another person — the true group/class photos she wants are all 10–21MB and stay blocked by that cap. Committed to `origin/dev` only, screenshotted and shown to Joshua, not yet pushed live. **v2 upgrade still needed:** compressed copies of the real group photos from Joshua/Narine, or another retrieval path.
